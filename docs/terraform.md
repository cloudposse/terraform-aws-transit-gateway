...
| config | Configuration for Transit Gateway, VPC attachments, Transit Gateway routes, and subnet routes | <pre>map(object({<br>    provider               = string<br>    vpc_id                 = string<br>    vpc_cidr               = string<br>    subnet_ids             = set(string)<br>    subnet_route_table_ids = set(string)<br>    static_routes = set(object({<br>      blackhole              = bool<br>      destination_cidr_block = string<br>    }))<br>    route_to = set(string)<br>  }))</pre> | n/a | yes |
